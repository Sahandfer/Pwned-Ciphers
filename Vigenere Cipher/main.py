...
def encrypt(text, key):
    ...
    for i in range(len(text)):
        ...
        encryptedText += chr((ord(text[i])+ord(key[keyIndex])-(textDecrement+keyDecrement) )% 26 + textDecrement)
    print(encryptedText)
...
def decrypt(cipherText, key):
    ...
    for i in range(len(cipherText)):
        ...
        keyDecrement = 65 if key[keyIndex].isupper() else 97
        decryptedText += chr((ord(cipherText[i])-ord(key[keyIndex])-(cipherDecrement-keyDecrement) )% 26 + cipherDecrement)
